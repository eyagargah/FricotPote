  ...
  sendMsg = async (e: any) => {
    ...
    if (this.selectedUser && chatInput.value !== '') {
      ...
      try {
        ...
        this.messageService
          .getMessages(this.senderId, this.recepientId)
          .then((data) => {
            this.userMessages = data;
            for (let i = 0; i <= this.userMessages.length; i++) {
              this.messages.push(
                this.messageService.formatMsg(this.userMessages[i])
              );
            }
        ...
        this.messageService.sortMessages(this.messages);
        this.messageService
          .getMessages(this.recepientId, this.senderId)
          .then((data) => {
            this.clickedUserMessages = data;
            for (let i = 0; i <= this.clickedUserMessages.length; i++) {
              this.messages.push(
                this.messageService.formatMsg(this.clickedUserMessages[i])
              );
            }
            this.messages = this.messageService.filterMessages(this.messages);
            this.messageService.sortMessages(this.messages);
